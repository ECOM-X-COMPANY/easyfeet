...
if (!customElements.get('compare-drawer')) {
  class CompareDrawer extends SideDrawer {
    ...
    updateBasketState() {
      ...
      if (compareCount === 1) {
        this.compareBasketArea.innerHTML += `<div class='text-sm compare-prompt js-select-more'>${theme.strings.compare.more} ${continueButton}</div>`;
      } else if (compareCount === 0) {
        this.compareBasketArea.innerHTML += `<div class='text-sm compare-prompt js-select-one'>${theme.strings.compare.empty} ${continueButton}</div>`;
      }
    }
