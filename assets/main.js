...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const randomCount = getRandomNumber(1, 100);
  randomNumberSpan.textContent = randomCount;
});


// Adds an inscription of the selected size to the size
document.addEventListener('DOMContentLoaded', function() {

  let legendElement = document.querySelector('.option-selector legend');

  let optLabels = document.querySelectorAll('.opt-label--btn');

  let selectedOption = document.querySelector('.opt-btn:checked');

  if (selectedOption) {
    let selectedText = selectedOption.nextElementSibling.textContent.trim();
    legendElement.textContent = 'Size: ' + selectedText;
  }

  optLabels.forEach(function(optLabel) {
    optLabel.addEventListener('click', function() {

      let labelText = this.querySelector('span').textContent;

      legendElement.textContent = 'Size: ' + labelText;
    });
  });
});
...
//The function of random numbers in reviews
document.addEventListener("DOMContentLoaded", () => {
  const generateRandomContent = () => {
    const randomNumber = Math.floor(Math.random() * 100);
    return `${randomNumber} Find this helpful`;
  };

  const createLiElement = (content) => {
    const liElement = document.createElement('li');
    const spanElement = document.createElement('span');
    spanElement.classList.add('random-number');
    spanElement.textContent = content.split(' ')[0];
    liElement.appendChild(spanElement);
    liElement.appendChild(document.createTextNode(content.slice(content.indexOf(' ') + 1)));
    liElement.classList.add('find-this-helpful');
    return liElement;
  };
  ...
  const observer = new MutationObserver((mutations) => {
    mutations.forEach((mutation) => {
      if (mutation.addedNodes && mutation.addedNodes.length > 0) {
        const ryviuWidget = document.getElementById("ryviu-widget4");
        if (ryviuWidget) {

          const replyHelpUlList = ryviuWidget.querySelectorAll('ul.r--reply-helpul');
          if (replyHelpUlList.length > 0) {

            replyHelpUlList.forEach((replyHelpUl) => {
              const content = generateRandomContent();
              const liElement = createLiElement(content);
              replyHelpUl.appendChild(liElement);
            });
...
// Replacing a hollow star with a half star
document.addEventListener("DOMContentLoaded", () => {
  const intervalId = setInterval(() => {
    const ryviuWidget = document.getElementById("ryviu-widget4");
    if (ryviuWidget) {
      const starIcons = ryviuWidget.querySelectorAll('.r-stbk');
      if (starIcons.length > 0) {
        starIcons.forEach((starIcon) => {
          starIcon.style.content = 'url("data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 id=%22Layer_1%22 data-name=%22Layer 1%22 viewBox=%220 0 24 24%22%3E%3Cpath d=%22m23.854,8.959c-.311-.941-1.18-1.948-2.688-1.948h-4.958l-1.515-5.032C14.229,.532,12.997,.017,12,.017s-2.229,.516-2.695,1.972l-1.515,5.023H2.83c-1.518,0-2.384,1.007-2.687,1.946-.298,.9-.211,2.27,1.017,3.165l4.05,2.96-1.543,5.209c-.469,1.448,.265,2.609,1.034,3.163.773,.559,2.111,.889,3.358-.035l3.938-3.325,3.964,3.345c.506,.372,1.063,.561,1.659,.561.587,0,1.195-.198,1.665-.541.774-.554,1.513-1.716,1.046-3.156l-1.546-5.222,4.049-2.959c1.22-.89,1.327-2.216,1.02-3.164Zm-1.609,2.355l-4.333,3.167c-.171,.125-.244,.343-.185,.546l1.649,5.572c.305,.938-.172,1.689-.677,2.049-.61,.445-1.51,.452-2.12,.006l-4.26-3.597c-.093-.079-.208-.118-.322-.118h0l.002-17.923h0c.312,0,1.363,.091,1.739,1.26l1.618,5.379c.063,.211,.258,.356,.479,.356h5.33c1.228,0,1.642,.965,1.738,1.259,.097,.297,.333,1.321-.659,2.044Z%22 fill=%22%23ff444f%22/%3E%3C/svg%3E")';
        });
  ...
  try {
    ...
    sizeButtons.forEach(function (button) {
      button.addEventListener('click', function () {
        ...
        const productPriceContainer = document.querySelector('.rbr-productNamePrice__price__container');

        if (!productPriceContainer) {
          throw new Error('No product price container found');
        }

        // Створюємо новий елемент <span> з класом 'price__was'
        const spanWas = document.createElement('span');
        spanWas.className = 'price__was';

        // Додаємо текст (або значення ціни) до <span>
        spanWas.innerText = variant.compare_at_price | money;

        // Додаємо <span> до контейнера продукту
        productPriceContainer.appendChild(spanWas);

        // Оновлюємо відображення ціни продукту
